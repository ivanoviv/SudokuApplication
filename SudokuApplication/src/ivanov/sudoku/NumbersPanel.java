...
/**
 * A class which creates the Sudoku board and inputs a game (fills in the numbers generated from the Game, or 
 * loaded by the user) on the board. The class also gathers the input from the user.
 * 
 * @author Ivo Ivanov
 *
 */
...
public class NumbersPanel extends JPanel {
	...
	public NumbersPanel() {
		...
		/**
		 * Creates the two dimensional array of TextFields to hold the number of the Sudoku
		 * Each TextField is placed on its respective JPanel (block)
		 */
		
		MaskFormatter f = createFormatter("#"); // the MaskFormatter is used to specify what can be input in the Text Fields 
		...
		for (int i = 0; i < numberFields.length; i++) {
			for (int j = 0; j < numberFields[i].length; j++) {
				...
				numberFields[i][j].addKeyListener(new DeleteValueListener(this.numberFields[i][j]));//adds an ActionListener used to delete the value from a textfield which is intended for user input
				panels[i / 3][j / 3].add(numberFields[i][j]);
			}
		}

		this.userInput = new boolean[9][9];
		this.setInitialGame(20); // Initial difficulty set to 20 which translates to easy
	}

	/**
	 * Creates a new Sudoku game and inputs the numbers in the Number Fields
	 *
	 *@param difficulty - a parameter indicating the difficulty of the the game
	 */
	public void setInitialGame(int dificulty) {
		...
		for (int i = 0; i < numberFields.length; i++) {
			for (int j = 0; j < numberFields[i].length; j++) {
				...
				this.setNumberInField(j, i, game.getNumber(j, i), this.userInput[i][j]);
	 ...
	 * @param game - an array of the current game to be set on the Sudoku board 
	 * @param userInput - a boolean matrix indicating whether the current cells is intended for the user ot input 
	 * @param savedGame - an object of type SavedGame, set as a instance field of the NumbersPanel
	 */
	 ...
	 * @param number - the number to be input in the field 
	 * @param userInput - a boolean which indicates whether the number is inputed by the user or by the game generator
	 * 	 
	 * */
	...
	/**
	 * calls the check method of the Game instance and colors the wrong input (the cells
	 * where the user inputted wrong numbers) by the user in red
	 * 
	 * @return boolean variable indicating whether the whole game was correct or not
	 */
	 ...
	 * 
	 * @return a two dimensional array with the all of the numbers in the Sudoku board
	 */
	 ...
	 * @return an obcjet of type MaskFormatter 
	 */
	...
	private class DeleteValueListener implements KeyListener {
		...
		public void keyPressed(KeyEvent e) {
			if (e.getKeyCode() == KeyEvent.VK_BACK_SPACE && this.numberField.getBackground() != Color.BLACK){
